...
A partir do framework v4, o estado global vive em `/Auditoria/_framework/audit-index.json` (substitui `status-geral.md` de versões anteriores). É um JSON validado pelo schema `audit-index.schema.json` em `schemas/runtime/`.

Verifique o estado atual do framework:

## Cenário A — Framework não existe
O diretório `/Auditoria/` não existe ou `/Auditoria/_framework/audit-index.json` não existe.

**Rota: iniciar pelo Setup (Etapa 2).**

## Cenário A2 — Formato legado pré-v4 detectado
`/Auditoria/_framework/` existe mas tem `status-geral.md` em vez de `audit-index.json`. Isso é instalação pré-v4.

```
Framework de Auditoria em formato legado (pré-v4) detectado.
A v4 introduziu estado JSON-first; o agente NÃO converte automaticamente para evitar perda de informação.

Para migrar:
- Use o utilitário `audkit migrate` (a ser disponibilizado), ou
- Finalize quaisquer runs em curso com a versão antiga e reinstale o framework v4 do zero.

Encerrando sem alterar arquivos.
```

**Encerre sem alterar arquivos.**

## Cenário B — Bootstrap Core existe, mas playbooks não
`/Auditoria/_framework/audit-index.json` existe, mas `/Auditoria/_framework/playbooks/` não existe ou está vazio.

Leia `audit-index.json` e verifique `playbooks_seed_status`:
- Se `"pending"` → **Rota: Etapa 3 (Seed Playbooks).**
- Se `"completed"` → inconsistência. Informe o usuário e sugira re-executar o seed.

## Cenário C — Framework completo, nenhuma run ativa
`audit-index.json` existe e todos os `domains[].status_current` são `"not_started"`.

Verifique se há correções interrompidas (procure por `correcao/correction.json` com `status: "em_andamento"` em qualquer `runs/<RUN_ID>/` de qualquer domínio).
...
Algum domínio em `audit-index.domains[]` está com `status_current` em `"in_progress"` ou `"blocked"`.
...
Após confirmar que os 16 playbooks + `index.md` foram criados em `/Auditoria/_framework/playbooks/` pelo Prompt 01B, remova a pasta `playbook/` original da raiz do projeto:
...
Opções:
- "todos" → auditar todos os 16 domínios em sequência
...
### Validação
- Se o usuário escolheu "todos": lista = todos os 16 domínios na ordem oficial
...
Leia `/Auditoria/_framework/audit-index.json` e busque a entrada `domains[<dominio>]`. A ordem de checagem importa — estados ativos têm prioridade sobre o caso "domínio com histórico":

### Se `status_current: "in_progress"`
Run ativa encontrada. Prossiga com 5.3 (Executar Run) para retomar.

### Se `status_current: "blocked"`
Informe o usuário sobre o bloqueio. Pergunte se deseja tentar retomar (Prompt 03 Etapa 2.1 conduz desbloqueio) ou pular este domínio.
...
### Se `status_current: "not_started"` E `ultima_run_finalizada != null` (domínio já auditado antes)

**Esta checagem vem ANTES do branch puro `not_started`** — domínio com histórico não deve iniciar nova run automaticamente sem oferecer alternativas.

Informe o usuário:

```
O domínio <DOMINIO> já possui auditoria finalizada (última run: <ULTIMA_RUN>).

Deseja:
1. Iniciar uma nova auditoria deste domínio (substitui current/ por uma nova run)
2. Pular para o próximo domínio (preserva histórico, nada muda)
3. Corrigir os achados da última auditoria (carrega Prompt 05 sobre <ULTIMA_RUN>)
```

Aguarde e prossiga conforme a escolha:
- **1** → prossiga com 5.2 (Iniciar Run).
- **2** → registre escolha e avance para o próximo domínio da lista.
- **3** → carregue Prompt 05 com pré-seleção de `<dominio>` e `<ULTIMA_RUN>`. Após concluir P05, **NÃO** retorne para iniciar nova auditoria — siga para o próximo domínio.

### Se `status_current: "not_started"` E `ultima_run_finalizada == null` (domínio nunca auditado)
Prossiga com 5.2 (Iniciar Run) sem perguntas.
...
O Prompt 05 possui seu próprio mecanismo de retomada (via `correction.json`). Se a correção foi interrompida anteriormente, o Prompt 05 detectará e retomará automaticamente.
...
Estado por run (consumido por dashboard):
- Auditoria:     /Auditoria/{{DOMINIO}}/runs/{{RUN_ID}}/report.json
- Achados:       /Auditoria/{{DOMINIO}}/runs/{{RUN_ID}}/findings.json
- Histórico:     /Auditoria/{{DOMINIO}}/runs/{{RUN_ID}}/phase-ledger.jsonl
- Correção:      /Auditoria/{{DOMINIO}}/runs/{{RUN_ID}}/correcao/correction-report.json

Estado global: /Auditoria/_framework/audit-index.json
Relatório consolidado: /Auditoria/_framework/report-consolidado.json
```
...
## Regra 3 — Estado nos arquivos JSON
O estado do pipeline está em arquivos JSON validados pelos schemas em `schemas/runtime/`:
- Estado global: `/Auditoria/_framework/audit-index.json`
- Estado da run ativa: `/Auditoria/<dominio>/current/{run,state,findings,report}.json` + `phase-ledger.jsonl`
- Estado da correção: `/Auditoria/<dominio>/runs/<RUN_ID>/correcao/correction.json`

Se o orquestrador for interrompido e reiniciado, ele detecta o estado lendo esses arquivos na Etapa 1 e retoma de onde parou. **Nunca confiar na memória da conversa** — apenas no estado em disco.
