...
<!-- GENERATED FILE — não edite manualmente. Edite playbook/*.md ou scripts/templates/prompt-01b.template.md e rode `npm run build:01b`. -->

# Prompt 01B — Seed Playbooks do Framework de Auditoria (v3 — JSON-first)

Você é um agente local operando diretamente no repositório do projeto-alvo.

Sua tarefa nesta execução é **somente** materializar o catálogo de playbooks e os **16 playbooks canônicos** do framework de auditoria já inicializado pelo **Prompt 01A — Bootstrap Core**.

# Estado JSON-first

A partir do framework v4, o estado operacional do framework vive em arquivos JSON validados pelos schemas em `schemas/runtime/`. O Bootstrap Core (Prompt 01A) já criou:

- `/Auditoria/_framework/audit-index.json` — estado global
- `/Auditoria/_framework/templates/*.template.json` — 7 templates JSON
- `/Auditoria/<dominio>/current/{run,state,findings,report}.json` + `phase-ledger.jsonl`
- `/Auditoria/<dominio>/runs/runs-index.json`

Os arquivos de docs internas em MD (`framework.md`, `convencoes.md`, etc.) **não existem mais** na v4 — foram removidos como parte da migração JSON-first. Apenas os logs de setup (`bootstrap-report.md`, `playbook-seed-report.md`) permanecem em Markdown.
...
- `ai-ml-governanca.playbook.md`

Além disso, esta execução deve:
- atualizar `/Auditoria/_framework/playbook-seed-report.md` (log de setup, MD)
- atualizar `/Auditoria/_framework/audit-index.json` (campo `playbooks_seed_status`)
- encerrar
...
Você deve:
1. validar que o Bootstrap Core já existe (audit-index.json + templates JSON)
...
5. criar ou completar os 16 playbooks canônicos usando o conteúdo canônico deste prompt
6. registrar o resultado em `/Auditoria/_framework/playbook-seed-report.md`
7. atualizar `/Auditoria/_framework/audit-index.json` (`playbooks_seed_status: "completed"`, `updated_at`)
8. encerrar
...
- inventar playbooks adicionais fora dos 16 oficiais
- resumir ou empobrecer o conteúdo canônico dos playbooks

# Modo de escrita permitido

Você pode escrever **somente** dentro de:
- `/Auditoria/_framework/playbooks/` — todos os arquivos do catálogo
- `/Auditoria/_framework/playbook-seed-report.md` — log de setup
- `/Auditoria/_framework/audit-index.json` — apenas para atualizar `playbooks_seed_status` e `updated_at`
...
- `/Auditoria/_framework/templates/run.template.json`
- `/Auditoria/_framework/templates/state.template.json`
- `/Auditoria/_framework/templates/findings.template.json`
- `/Auditoria/_framework/templates/report.template.json`
- `/Auditoria/_framework/templates/runs-index.template.json`
- `/Auditoria/_framework/templates/correction.template.json`
- `/Auditoria/_framework/templates/correction-report.template.json`

Validações adicionais ao ler `audit-index.json`:
- arquivo é JSON válido
- `format_version: 1`
- `domains` é array com 16 entradas

Se qualquer pré-condição falhar, **aborte sem improvisar**. O Bootstrap Core (Prompt 01A) deve ser executado primeiro.
...
# Conteúdo canônico dos 16 playbooks
...
Os playbooks definem objetivos, fases e critérios de auditoria. Algumas seções históricas podem mencionar arquivos `.md` operacionais (ex.: `relatorio-final.md`, `achados.md`) — esses nomes são **referências semânticas** ao estado da run; no framework v4, equivale aos arquivos JSON correspondentes (`report.json`, `findings.json`, etc.). O Prompt 03 (Executar Run) traduz essas referências automaticamente ao escrever o estado.
...
## Playbook: `ai-ml-governanca.playbook.md`

Crie `/Auditoria/_framework/playbooks/ai-ml-governanca.playbook.md` com o seguinte conteúdo:
...
# Atualização obrigatória do `audit-index.json`

Ao final da execução, atualize `/Auditoria/_framework/audit-index.json`:

- `playbooks_seed_status`: `"completed"` se todos os 16 playbooks oficiais estiverem criados e o `index.md` populado
- `playbooks_seed_status`: `"pending"` se houver conflitos, faltas ou incompatibilidades (e detalhe em `playbook-seed-report.md`)
- `updated_at`: timestamp ISO 8601 atual

**Não altere** as entradas em `domains[]` — esta etapa não muda status de domínio nem zera runs/histórico. Apenas o flag global de seed e o `updated_at` são tocados.

# Atualização obrigatória do `playbook-seed-report.md`

Atualize `/Auditoria/_framework/playbook-seed-report.md` com:

- `data_hora_execucao`: timestamp real
- `status_resultado`: `completed` ou `partial`
- `seed_executado`: `sim` ou `nao` (em status_resultado=partial, registre `parcial`)
- `total_playbooks_criados`: número real
- catálogo (`index.md`) criado ou preservado: registre
- lista de playbooks criados, preservados, completados
...
7. Não criar playbooks extras fora dos 16 oficiais.
...
12. Não tocar em `domains[]` em `audit-index.json` — apenas `playbooks_seed_status` e `updated_at` são alterados aqui.
...
2. os 16 playbooks oficiais estiverem criados com o conteúdo canônico completo deste prompt
3. `/Auditoria/_framework/playbook-seed-report.md` estiver atualizado
4. `/Auditoria/_framework/audit-index.json` estiver atualizado (`playbooks_seed_status: "completed"` e `updated_at` atual)
