...
### 4.6 — Regenerar o relatório consolidado (report-consolidado.json)

Após persistir o progresso da fase e **antes** de avançar para a próxima, regenere o relatório consolidado — isso mantém dashboards externos (Framework-Dashboard) atualizados com o progresso em tempo real.

**Identifique `{{RAIZ_PROJETO_ALVO}}`** — é o caminho absoluto do diretório que contém `/Auditoria/`. Você pode obter com:

```bash
# Se /Auditoria/ está em algum ancestral do cwd atual:
...
A flag `--include-current` garante que os achados da run em progresso (ainda não finalizada) apareçam no report. O comando gera/atualiza:

- `/Auditoria/_framework/report-consolidado.json` — formato máquina, consumido por dashboards.
- `/Auditoria/_framework/report-consolidado.md` — formato humano.

**Tratamento de erro:**

- Se `.audkit` não existir na raiz do projeto-alvo (instalação pré-v3.3.0): pule esta sub-etapa silenciosamente e siga. Não falhe a run por causa disso. O report será gerado no Prompt 04 quando a run for finalizada.
- Se `.audkit` existir mas falhar (ex.: Node ausente, erro de leitura): registre uma linha em `acompanhamento.md` sinalizando a falha e siga. A run principal não deve ser interrompida por falha do report.

**Commit automático: NÃO.** Esta sub-etapa **apenas atualiza os arquivos locais**. O usuário é responsável por commitar/pushar quando quiser que o dashboard remoto veja a atualização. Para automação de commit por fase, o usuário pode configurar um git hook próprio — não é responsabilidade deste prompt.
