...
# Etapa 10 — Geração do relatório consolidado (para dashboards externos)

Após arquivar a run com sucesso, gere o arquivo `/Auditoria/_framework/report-consolidado.json` via CLI `audkit report`. Esse arquivo agrega achados de **todas** as runs finalizadas e é o que dashboards externos (ex.: Framework-Dashboard) leem para monitorar o projeto.

## Como invocar o audkit

O wrapper da CLI é instalado pelo `install.sh` na raiz do projeto como `.audkit`.

**Identifique `{{RAIZ_PROJETO_ALVO}}`** — é o caminho absoluto do diretório que contém `/Auditoria/`:
...
Depois execute com paths absolutos (workdir-independent):
...
- `/Auditoria/_framework/report-consolidado.md` — agregado legível por humanos

## Se o wrapper não existir

Pode acontecer em instalações mais antigas (pré-v3.3.0). Duas opções:

1. **Atualizar o framework** — reinstalar via `install.sh` do repositório do framework cria o wrapper `.audkit`.
2. **Invocar diretamente** — se você souber o caminho absoluto do repositório do framework no computador, execute:
   ```bash
   node <FRAMEWORK_PATH>/cli/src/index.mjs report .
   ```

Se não souber o path, pergunte ao usuário. Se mesmo assim não for possível, **pule esta etapa** e registre em `acompanhamento.md` que o relatório consolidado não foi gerado nesta execução por indisponibilidade da CLI. A run em si foi arquivada com sucesso; o dashboard externo não será atualizado até que `audkit report` rode manualmente depois.

## Após gerar

Verifique que os dois arquivos foram criados e estão bem-formados (`report-consolidado.json` deve ser JSON válido). Em seguida, prossiga para a Etapa 11.

---

# Etapa 11 — Confirmação final ao usuário
...
8. O `report-consolidado.json` foi gerado em `/Auditoria/_framework/` (ou a ausência foi explicitamente registrada se a CLI audkit não estava disponível)
9. O usuário recebeu a confirmação final com o resumo e os próximos passos possíveis
