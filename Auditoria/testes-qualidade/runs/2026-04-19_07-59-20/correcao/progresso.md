...
- nota_revisor: Bloco 9 do roadmap (linhas 113-123) lista os 6 módulos órfãos exatos do ACH-012 (`analytics`, `ai`, `campaigns`, `landing`, `schedule`, `team`) com ordem de priorização justificada por exposição a bugs em produção — 1º campaigns (dispatch multi-tenant), 2º schedule (timezone bug farm), 3º analytics (queries agregadoras), 4º ai/landing/team. Aprovação parcial adequada pelo constraint CLAUDE.md "ZERO testes até Fase 7".
