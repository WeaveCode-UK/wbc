...
import { listAppointments } from "../lib/offline-repos";
...
const EVENTS: TimelineEvent[] = [
  {
    ...
    description: "Full skin analysis and product recommendation",
    location: "Atelier Studio B",
    duration: "1h 30min",
  ...
  },
  {
    ...
    description: "Order #1847 — Kit TimeWise + Serum",
  ...
  },
  {
    ...
    description: "New skincare line demo for VIP group",
    location: "Atelier Studio A",
    duration: "2h",
    category: "demo",
  },
  {
...
function categoryFromType(type: string): CategoryKey {
...
export function ScheduleScreen() {
  ...
  useEffect(() => {
    let cancelled = false;
    void listAppointments(tenantId).then((rows) => {
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitle, { color: c.outline }]}>
          Managing 12 upcoming atelier sessions
        </Text>
      </View>
      ...
      <View style={styles.dateNav}>
        <TouchableOpacity
          activeOpacity={0.7}
          style={[styles.navArrow, { backgroundColor: c.surfaceContainerLow }]}
        >
          <Text
            style={{
        ...
        <Text style={[styles.dateLabel, { color: c.onSurface }]}>
          October 24, 2023
        </Text>
        <TouchableOpacity
          activeOpacity={0.7}
                  ...
                  {dotColor ? (
                    <View
                      style={[
                        styles.eventDot,
                        {
                          backgroundColor: isSelected ? c.onPrimary : dotColor,
                        },
                      ]}
                    />
                  ) : null}
      ...
      <View style={styles.legendRow}>
        {(
          Object.entries(CATEGORIES) as Array<[CategoryKey, { label: string }]>
        ).map(([key, val]) => (
          <View key={key} style={styles.legendItem}>
            ...
            />
            <Text style={[styles.legendLabel, { color: c.outline }]}>
              {val.label}
            </Text>
          </View>
        ))}
      ...
      <View style={styles.timeline}>
        ...
        <View style={[styles.currentTimeRow]}>
          ...
          <View
            style={[styles.currentTimeLine, { backgroundColor: c.error }]}
          />
          ...
          return (
            <View key={idx} style={styles.timelineItem}>
              ...
              <View style={styles.timeColumn}>
                <Text style={[styles.timeLabel, { color: c.outline }]}>
                  {event.time}
                </Text>
              </View>
              {/* Dot */}
              <View
                style={[
                  styles.timelineDot,
                  { backgroundColor: accentColor, borderColor: c.surface },
                ]}
              />
              {/* Event Card */}
              <View
              ...
              >
                ...
                <Text style={[styles.eventDescription, { color: c.outline }]}>
                  {event.description}
                </Text>
                <View style={styles.locationRow}>
                  ...
                  <Text style={[styles.locationText, { color: c.outline }]}>
                    {event.location}
                  </Text>
                </View>
              </View>
