...
async function processCampaignJob(job: Job<CampaignJob>): Promise<void> {
  ...
  logger.info({ jobId: job.id, jobName: job.name, campaignId, tenantId }, 'Processing campaign job');

  await withTenant(tenantId, async () => {
    if (job.name === 'send-campaign') {
      const recipients = await campaignRepo.getRecipients(tenantId, campaignId, 'PENDING');
      logger.info({ campaignId, recipientCount: recipients.length }, 'Dispatching campaign messages');
      await campaignRepo.updateStatus(tenantId, campaignId, 'SENDING');

      for (const recipient of recipients) {
        logger.info({ campaignId, clientId: recipient.clientId }, 'Sending to recipient');
