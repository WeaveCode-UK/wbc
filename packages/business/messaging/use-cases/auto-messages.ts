...
export async function handlePaymentOverdue(tenantId: string, clientId: string, amount: number, saleId: string, repo: ScheduledMessageRepository): Promise<void> {
...
export async function handleClientCreated(tenantId: string, clientId: string, source: string, repo: ScheduledMessageRepository): Promise<void> {
...
export async function handleCashbackExpiring(tenantId: string, clientId: string, amount: number, daysLeft: number, repo: ScheduledMessageRepository): Promise<void> {
  await repo.create(tenantId, {
    clientId,
...
export function registerAutoMessageHandlers(repo: ScheduledMessageRepository): void {
  subscribe(EVENTS.PAYMENT_OVERDUE, async (event) => {
    ...
    await handlePaymentOverdue(p.tenantId, p.clientId, p.amount, p.saleId, repo);
  });
  ...
  subscribe(EVENTS.CASHBACK_EXPIRING, async (event) => {
    ...
    await handleCashbackExpiring(p.tenantId, p.clientId, p.amount, p.daysLeft, repo);
  });
}
